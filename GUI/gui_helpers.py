...
def process_log_file(tlog_file_path_str):
    ...
    # Use a temporary directory for the CSV
...
def generate_pdf_export(output_pdf_path, log_file_name, summary_stats, timeline_data,
    ...
    # Ensure plot directory exists (for temp files)
    ...
    # Save fig objects
    # map_fig should now be the one with the basemap from the GUI
    if map_fig: map_fig.savefig(map_path, bbox_inches='tight')
    ...
    reporting.generate_forensic_report(
        output_pdf_path=str(output_pdf_path), # Ensure it's a string
